		...
		return t.run( new Runnable() {
			public void run() {
				try {
					...
					LoggerFactory.getLogger(this.getClass()).error("Error closing conenction: " + e.getMessage());
				}						
			}
		}, timeout);
	}
		...
		return t.run( new Runnable() {
			...
			public void run() {
				try {
					...
					LoggerFactory.getLogger(this.getClass()).error("Error shutting down repository: " + e.getMessage());
				}						
			}
		}, timeout);
	}
