...
import org.aksw.simba.fedsum.FedSumSourceSelection.CheckTaskPair;
import org.aksw.simba.fedsum.hypergraph.HyperGraph.HyperEdge;
import org.aksw.simba.fedsum.hypergraph.HyperGraph.Vertex;
...
public class SourceSelection {
	 ...
	 */
	public static  Map<StatementPattern, List<StatementSource>> stmtToSources = new ConcurrentHashMap<StatementPattern, List<StatementSource>>();
	...
	public void doSourceSelection(List<StatementPattern> stmts) throws RepositoryException, MalformedQueryException, QueryEvaluationException {
	 long tp = 0 ;
	 for(int DNFkey:QueryEvaluation.DNFgrps.keySet())
	  {
		 ...
		 //    HashSet<HyperEdge> E = new HashSet<HyperEdge>();  //-- set of all hyperedges. an edge contains subject, predicate and object node of a triple pattern
			 ...
			 List<StatementPattern> gstmts = QueryEvaluation.DNFgrps.get(DNFkey);  
		...
		for (StatementPattern stmt : stmts) 
		{
			...
			if(FedSumConfig.mode.equals("ASK_dominant"))   //---ASK_dominant algo
			{
	          ...
	          else if (!p.equals("null"))
	          {
	        	  ...
	        	  else if (!FedSumConfig.commonPredicates.contains(p) || (s.equals("null") && o.equals("null")))
...
//				//	System.out.println(stmt+"--cache->"+e.getId());
//				} else if (a==StatementSourceAssurance.HAS_REMOTE_STATEMENTS) {
//					addSource(stmt, new StatementSource(e.getId(), StatementSourceType.REMOTE));
//					//System.out.println(stmt+"-cache, no ask-->"+e.getId());
//				} 
//			else if (a==StatementSourceAssurance.POSSIBLY_HAS_STATEMENTS) {	
//					//System.out.println(stmt+"--not in caches, ASK->"+e.getId());
...
//		}
	  ...
	  int triplePatternWiseSources = 0 ;
		for (StatementPattern stmt : stmtToSources.keySet()) {
		 ...
		 System.out.println("Total Triple Pattern-wise selected sources after step 1: " + triplePatternWiseSources);
...
//		}
	...
	/**
	 * Step 2 of FedSum source selection. i.e. triple pattern-wise selected sources for hyperedge aka triple pattern
	 * @param dNFHyperVertices DNF groups of hypervertices
 ...
 */
	...
	{
		String endPointUrl = "http://"+src.getEndpointID().substring(7);//replace("sparql_", "");
 ...
 */
	public String getFedSumSbjAuthLookupQuery(StatementPattern stmt,String endPointUrl)
	{
		...
		if (stmt.getSubjectVar().getValue()!=null) //if subject is bound
		{
			s = stmt.getSubjectVar().getValue().stringValue();
			String[] sbjPrts = s.split("/");
			sa =sbjPrts[0]+"//"+sbjPrts[2];
		...
		if (stmt.getPredicateVar().getValue()!=null) //--if predicate is bound
		p = stmt.getPredicateVar().getValue().stringValue();
		else
			p ="null"; 
...
//	{
	...
	public void cache_ASKselection(StatementPattern stmt)
	{
		...
		/**
		 * Search index for the given triple pattern p with sbj authority and obj authority.
