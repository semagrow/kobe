...
public class FedXHibiscusSparqlEvaluation extends SesameSparqlEvaluation{

	private boolean isInitialized = false;
	private FedXMonitoringReport fedxReport = null;
	
	public FedXHibiscusSparqlEvaluation() {
		super();
	}
	...
	public void initialize() throws Exception {
		...
		if (!isInitialized) {
			String FedSummaries = Config.getConfig().getProperty("summariesFile");
			String mode = Config.getConfig().getProperty("hibiscusMode");
			int commonPredThreshold = Integer.parseInt(Config.getConfig().getProperty("commonPredThreshold"));
		
			FedSumConfig.initialize(FedSummaries,mode,commonPredThreshold);
			isInitialized = true;
		}
		...
		sailRepo = FedXFactory.initializeSparqlFederation(FedSumConfig.dataSources);
